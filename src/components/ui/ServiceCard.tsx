'use client';
import { Globe, MapPin, Star, CreditCard, MessageCircle } from 'lucide-react';
        ...
        >
            <div className="relative h-48 w-full bg-gray-200">
                ...
                <div className="absolute top-3 left-3 right-12 flex flex-wrap gap-1.5 z-10 items-start pointer-events-none">
                    {isRemote ? (
                        <div className="bg-black/80 backdrop-blur text-white px-2 py-0.5 md:px-2.5 md:py-1 rounded-lg text-[10px] md:text-xs font-semibold flex items-center gap-1 shadow-sm shrink-0 max-w-full">
                            <Globe size={10} className="shrink-0" /><span className="truncate">Zdalnie</span>
                        </div>
                    ) : (
                        <div className="bg-black/80 backdrop-blur text-white px-2 py-0.5 md:px-2.5 md:py-1 rounded-lg text-[10px] md:text-xs font-semibold flex items-center gap-1 shadow-sm shrink-0 max-w-full">
                            <MapPin size={10} className="shrink-0" /><span className="truncate">{service.city}</span>{service.radius > 0 ? ` +${service.radius}km` : ''}
                        </div>
                    )}
                ...
                <div className="absolute bottom-3 right-3 z-10">
                    <div className="bg-white px-3 py-2 md:px-4 md:py-3 rounded-xl shadow-sm text-right">
                ...
                <div className="flex items-start justify-between mb-2 md:mb-3">
                    <div className="flex items-center gap-2 md:gap-3">
                        <div className="relative">
                            <UserAvatar src={service.provider.avatar} name={service.provider.name} size={36} className="rounded-full border-2 border-white shadow" />
                            {isUserActive && <span className="absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full ring-2 ring-white bg-green-500" />}
                        </div>
                    ...
                    <div className="flex items-center gap-1 bg-amber-50 text-amber-900 px-2 py-1 md:px-3 md:py-1.5 rounded-lg">
                        <Star size={12} fill="currentColor" /><span className="font-bold text-xs md:text-sm">{service.rating}</span>
                    </div>
                </div>
